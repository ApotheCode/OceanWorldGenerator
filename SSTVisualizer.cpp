...
bool loadClimateData(const std::string& filename, std::vector<double>& data)
{
    ...
    std::cout << "📦 Climate data: " << width << "×" << height << "\n";
    std::cout << "   Header depth: " << header.depth << " (using 14 variables)\n";
    std::cout << "   File size: " << fileSize << " bytes\n";
    
    // Calculate expected size
    size_t headerSize = 28;
    size_t totalDataSize = (size_t)WIDTH * HEIGHT * VARIABLES * MONTHS;
    size_t expectedFileSize = headerSize + totalDataSize * sizeof(double);
                     ...
                     std::vector<unsigned char>& isLand)
{
    ...
    ClimateFileHeader header;
    if (!readClimateHeader(file, header)) {
        std::cerr << "⚠️ Failed to read terrain header\n";
        return false;
    }
    ...
    if (width != WIDTH || height != HEIGHT) {
        std::cerr << "⚠️ Terrain dimension mismatch: Expected " << WIDTH << "x" << HEIGHT 
                  << ", got " << width << "x" << height << "\n";
        return false;
    }
