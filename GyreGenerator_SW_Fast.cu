    ...
    int width, int height)
{
    ...
    double H_b      = H[idx];
    double H_tot    = H_b + eta_curr;
    if (H_tot < params.H_min) H_tot = params.H_min;
    ...
    double dudt_local = tau_x / (params.rho0 * H_tot);

    dudt[idx]   = dudt_local;
    dvdt[idx]   = 0.0;
    detadt[idx] = 0.0;

    if (step == 0 && x == width / 2 && y == height / 2) {
        printf("KERNEL tau-only: lat=%g deg, tau_x=%g, H_tot=%g, dudt=%g\n",
               lat * 180.0 / M_PI, tau_x, H_tot, dudt_local);
    }
}
...
if (step == 0) {
    ...
    // Scan tendencies to find max |dudt| and basic stats
    ...
    // === New equator sanity check using real bathymetry ===
    int y_equator   = HEIGHT / 2;
    int x_equator   = WIDTH  / 2;
    int idx_equator = y_equator * WIDTH + x_equator;

    double lat_equator = (y_equator - HEIGHT / 2.0) * params.dlat;  // same as device
    double tau_x_equator = 0.0;
    double tau_y_equator = 0.0;

    getWindStress(lat_equator, month, tau_x_equator, tau_y_equator, params);

    double H_equator            = h_H_check[idx_equator];
    double expected_dudt_equator = tau_x_equator / (params.rho0 * H_equator);

    printf("DEBUG (Equator):\n");
    printf("   lat = %.2f deg\n", lat_equator * 180.0 / M_PI);
    printf("   tau_x = %.6g N/m^2\n", tau_x_equator);
    printf("   tau_y = %.6g N/m^2\n", tau_y_equator);
    printf("   H = %.2f m\n", H_equator);
    printf("   expected dudt = %.6e m/s^2\n", expected_dudt_equator);

    // Summary for the whole field
    std::cout << "\n📊 DEBUG: Max |dudt| after fused_dynamics = "
              << max_dudt << " m/s² (diagnostic range ~1e-8–1e-6 for this setup)\n";
    std::cout << "   Ocean cells in mask: " << ocean_cells << " / " << N << "\n";
    std::cout << "   NaN ocean cells in dudt: " << nan_count << "\n";

    // Detailed info at the cell where |dudt| is maximum
    if (max_i >= 0) {
        int x = max_i % WIDTH;
        int y = max_i / WIDTH;

        double lat = (y - HEIGHT / 2.0) * params.dlat;
        double test_tau_x = 0.0;
        double test_tau_y = 0.0;

        getWindStress(lat, month, test_tau_x, test_tau_y, params);

        double H_local       = h_H_check[max_i];
        double expected_dudt = test_tau_x / (params.rho0 * H_local);

        std::cout << "   Cell with max dudt: (" << x << "," << y
                  << ") lat=" << (lat * 180.0 / M_PI) << "°\n";
        std::cout << "   Wind stress: tau_x=" << test_tau_x << " N/m²\n";
        std::cout << "   Expected dudt: " << expected_dudt << " m/s²\n";
        std::cout << "   Actual dudt: "   << h_k1_u[max_i] << " m/s²\n";
        std::cout << "   Bathymetry: "    << H_local << " m\n";
    }
...
int main(int argc, char** argv)
{
    ...
    params.spin_up_days = 1825;  // 5 years
