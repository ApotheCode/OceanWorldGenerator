    ...
    int width, int height)
{
    ...
    // Vorticity from velocity derivatives
    ...
    // *** INCREMENTAL TEST: Wind + Pressure + Bottom Drag + Coriolis ***
    double dudt_val = 0.0;
    double dvdt_val = 0.0;
    
    // 1. Wind term (confirmed working)
    double tau_x, tau_y;
    ...
    double H_wind = 100.0;
    dudt_val += tau_x / (params.rho0 * H_wind);
    dvdt_val += tau_y / (params.rho0 * H_wind);
    
    // 2. Pressure gradient (confirmed working)
    double detadx = ddx_shared(s_eta, tx, ty, dx);
    double detady = ddy_shared(s_eta, tx, ty, dy);
    
    dudt_val -= params.g * detadx;
    dvdt_val -= params.g * detady;
    
    // 3. Bottom drag (confirmed zero at t=0)
    dudt_val -= params.r_bottom * u_curr;
    dvdt_val -= params.r_bottom * v_curr;
    
    // 4. Coriolis + vorticity (TESTING THIS - should be zero at t=0)
    dudt_val += (f + zeta) * v_curr;
    dvdt_val -= (f + zeta) * u_curr;
    
    // Assign
    dudt[idx]   = dudt_val;
    dvdt[idx]   = dvdt_val;
    detadt[idx] = 0.0;  // Not testing continuity yet
}
