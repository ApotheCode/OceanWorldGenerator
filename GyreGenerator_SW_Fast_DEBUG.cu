...
{
    // Shared memory tiles (with halo for neighbors)
    __shared__ double s_u[TILE_WITH_HALO][TILE_WITH_HALO];
    __shared__ double s_v[TILE_WITH_HALO][TILE_WITH_HALO];
    __shared__ double s_eta[TILE_WITH_HALO][TILE_WITH_HALO];
    __shared__ double s_H[TILE_WITH_HALO][TILE_WITH_HALO];
    __shared__ unsigned char s_mask[TILE_WITH_HALO][TILE_WITH_HALO];
    ...
    // Load tile with halo (coalesced reads)
    for (int dy = 0; dy < TILE_WITH_HALO; dy += TILE_SIZE) {
        for (int dx = 0; dx < TILE_WITH_HALO; dx += TILE_SIZE) {
            int sx = tx + dx;
            int sy = ty + dy;
            
            if (sx < TILE_WITH_HALO && sy < TILE_WITH_HALO) {
                int gx = (blockIdx.x * TILE_SIZE + sx - 1 + width) % width;
                int gy = blockIdx.y * TILE_SIZE + sy - 1;
    ...
    // Current state (from shared memory)
    double u_curr = s_u[ty + 1][tx + 1];
    double v_curr = s_v[ty + 1][tx + 1];
    double H_curr = fmax(s_H[ty + 1][tx + 1], params.H_min);
    
    // Compute Coriolis and vorticity
    double f = coriolisF(lat, params);
    
    // Velocity derivatives for vorticity and advection
    double dudx = ddx_shared(s_u, tx, ty, dx);
    double dudy = ddy_shared(s_u, tx, ty, dy);
    double dvdx = ddx_shared(s_v, tx, ty, dx);
    double dvdy = ddy_shared(s_v, tx, ty, dy);
    double zeta = dvdx - dudy;
    
    // Pressure gradient
    double detadx = ddx_shared(s_eta, tx, ty, dx);
    double detady = ddy_shared(s_eta, tx, ty, dy);
    ...
    // Viscosity
    double visc_u = params.A_h * laplacian_shared(s_u, tx, ty, dx, dy);
    double visc_v = params.A_h * laplacian_shared(s_v, tx, ty, dx, dy);
    ...
    double H_wind = 100.0;  // Surface mixed layer depth
    
    // MOMENTUM EQUATIONS (full shallow water)
    // Compute each term separately for debugging
    double wind_term_u = tau_x / (params.rho0 * H_wind);
    double pressure_term_u = -params.g * detadx;
    double coriolis_term_u = (f + zeta) * v_curr;
    double advection_term_u = -advect_u;
    double friction_term_u = -params.r_bottom * u_curr;
    double viscosity_term_u = visc_u;
    
    dudt[idx] = wind_term_u + pressure_term_u + coriolis_term_u 
                + advection_term_u + friction_term_u + viscosity_term_u;
    ...
    // DEBUG at one cell at step 0
    if (step == 0 && x == 1800 && y == 900) {  // Mid-latitude, mid-longitude
        printf("TERM DEBUG at (%d,%d):\n", x, y);
        printf("  wind_term = %.10e\n", wind_term_u);
        printf("  pressure_term = %.10e\n", pressure_term_u);
        printf("  coriolis_term = %.10e\n", coriolis_term_u);
        printf("  advection_term = %.10e\n", advection_term_u);
        printf("  friction_term = %.10e\n", friction_term_u);
        printf("  viscosity_term = %.10e\n", viscosity_term_u);
        printf("  TOTAL dudt = %.10e\n", dudt[idx]);
        printf("  u_curr = %.10e, v_curr = %.10e\n", u_curr, v_curr);
        printf("  tau_x = %.10e, rho0 = %.1f, H_wind = %.1f\n", tau_x, params.rho0, H_wind);
    }
    
    // CONTINUITY EQUATION
    double D_xp = (s_H[ty + 1][tx + 2] + s_eta[ty + 1][tx + 2]);
    double D_xm = (s_H[ty + 1][tx] + s_eta[ty + 1][tx]);
    double Du_xp = D_xp * s_u[ty + 1][tx + 2];
    double Du_xm = D_xm * s_u[ty + 1][tx];
    double dDudx = (Du_xp - Du_xm) / (2.0 * dx);
    
    double dDvdy = 0.0;
    if (y > 0 && y < height - 1) {
        double D_yp = (s_H[ty + 2][tx + 1] + s_eta[ty + 2][tx + 1]);
        double D_ym = (s_H[ty][tx + 1] + s_eta[ty][tx + 1]);
        double Dv_yp = D_yp * s_v[ty + 2][tx + 1];
        double Dv_ym = D_ym * s_v[ty][tx + 1];
    ...
    PhysicsParams params)
{
    ...
    for (int step = 0; step < total_steps; step++) {
        ...
        if (step == 0) {
            ...
            std::cout << "   NaN ocean cells in dudt: " << nan_count << "\n";
            ...
            if (nan_count > 0) {
                std::cout << "   ⚠️  WARNING: " << nan_count << " cells have NaN tendencies!\n";
                std::cout << "   ⚠️  This suggests division by zero (likely at poles where dx→0)\n";
            }
        }
