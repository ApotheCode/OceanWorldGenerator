...
const int WIDTH = STANDARD_WIDTH;
const int HEIGHT = STANDARD_HEIGHT;
...
bool convertRainfallToBinary(const std::string& inputFolder, const std::string& outputFile) {
    ...
    // Create standard header for rainfall data (12 monthly values)
    ClimateFileHeader header;
    header.magic = CLIMATE_MAGIC;
    header.width = WIDTH;
    header.height = HEIGHT;
    header.channels = 12;  // 12 months of rainfall
    header.dtype = 1;      // float type
    header.version = STANDARD_VERSION;
    header.depth = 12;     // 12 months
...
bool convertElevationToBinary(const std::string& inputFolder, const std::string& outputFile) {
    ...
    // Create standard header for elevation data (single channel)
    ClimateFileHeader header;
    header.magic = CLIMATE_MAGIC;
    header.width = WIDTH;
    header.height = HEIGHT;
    header.channels = 1;   // Single elevation layer
    header.dtype = 1;      // float type
    header.version = STANDARD_VERSION;
