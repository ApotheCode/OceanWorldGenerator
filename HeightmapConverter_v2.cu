...
int main(int argc, char** argv) {
    ...
    float northLat = 37.5f;  // Northern boundary (degrees North)
    float southLat = 22.5f;  // Southern boundary (degrees North - NOT southern hemisphere!)
    ...
    std::cout << "Latitude range: " << northLat << "°N to " << southLat << "°N" << std::endl;
    ...
    int endY = (int)((90.0f - southLat) * 10.0f); // FIXED: Both boundaries use northern hemisphere convention
    ...
    // Write binary file with standard 28-byte header
    ...
    // Create standard header for terrain data
    ClimateFileHeader header;
    header.magic = TERRAIN_MAGIC;  // Use proper TERRAIN_MAGIC
    header.width = outputWidth;
    header.height = outputHeight;
    header.channels = 1;  // Single channel terrain data
    header.dtype = 1;     // float type
    header.version = STANDARD_VERSION;
    header.depth = 1;     // No temporal dimension
    
    writeClimateHeader(outFile, header);
    
    // Write elevation and land mask data
