...
TerrainData loadTerrainData(const std::string& filename)
{
    ...
    if (width != WIDTH || height != HEIGHT) {
        std::cerr << "❌ Terrain dimensions mismatch!\n";
        exit(EXIT_FAILURE);
    }
    ...
    std::vector<float> h_oceanDepth(totalPixels);
    std::vector<float> h_continentality(totalPixels);
    ...
    file.read(reinterpret_cast<char*>(h_oceanDepth.data()), totalPixels * sizeof(float));
    file.read(reinterpret_cast<char*>(h_continentality.data()), totalPixels * sizeof(float));
...
int main(int argc, char** argv)
{
    ...
    std::vector<double> h_climateData(totalDataSize);
    climFile.read(reinterpret_cast<char*>(h_climateData.data()), 
                 totalDataSize * sizeof(double));
    climFile.close();
    
    std::cout << "📦 Loaded climate data: " 
              << (totalDataSize * sizeof(double) / (1024.0*1024.0*1024.0)) << " GB\n\n";
    ...
    outFile.write(reinterpret_cast<const char*>(h_climateData.data()), 
                 totalDataSize * sizeof(double));
    outFile.close();
