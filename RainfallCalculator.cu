...
bool loadClimateData(const std::string& filename, std::vector<double>& data)
{
    ...
    std::cout << "📦 Climate: " << depth << "×" << height << "×" << width << "\n";
    
    // Read all data
    size_t totalSize = width * height * VARIABLES * MONTHS;
    data.resize(totalSize);
    file.read(reinterpret_cast<char*>(data.data()), totalSize * sizeof(double));
                    ...
                    std::vector<unsigned char>& isLand)
{
    ...
    if (width != WIDTH || height != HEIGHT) {
        std::cerr << "❌ Terrain dimension mismatch\n";
        return false;
    }
...
bool saveClimateData(const std::string& filename, const std::vector<double>& data)
{
    ...
    int height = HEIGHT;
    int width = WIDTH;
    file.write(reinterpret_cast<const char*>(&depth), sizeof(int));
