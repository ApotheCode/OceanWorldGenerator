    ...
    int width, int height)
{
    ...
    // Check mask directly (NO shared memory)
    if (mask[idx] == 0) {
        dudt[idx] = 0.0;
        dvdt[idx] = 0.0;
        detadt[idx] = 0.0;
    ...
    // Read velocities directly from global memory
    double u_curr = u[idx];
    double v_curr = v[idx];
    ...
    // MOMENTUM EQUATIONS - simple direct assignment like original
    dudt[idx] = tau_x / (params.rho0 * H_wind)
                - params.r_bottom * u_curr;
    
    dvdt[idx] = tau_y / (params.rho0 * H_wind)
